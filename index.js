...
const DEV_SPACE_CHANNEL_ID = process.env.DEV_SPACE_CHANNEL_ID;
...
// Voice channels using new naming
...
const CLAN_ROLE_ID = process.env.CLAN_ROLE_ID || '1358789036558254293';
const GUEST_ROLE_ID = process.env.GUEST_ROLE_ID || '1358771756038492273';
...
async function sendMeetingReminder(topic, meetingId) {
    try {
        ...
        const generalChannel = await client.channels.fetch(GENERAL_VOICE_CHANNEL_ID);
        const channelLink = `https://discord.com/channels/${channel.guildId}/${GENERAL_VOICE_CHANNEL_ID}`;
        
        const reminderMessage = `📢 **MEETING IS NOW LIVE!**

📝 **Topic:** ${topic}
🎙️ **Location:** ${generalChannel.name}
🔗 **Join here:** ${channelLink}

⚡ All Adepti are requested to join now!`;
            ...
            for (const person of todaysBirthdays) {
                try {
                    ...
                    const wishMessage = 
                        ...
                        `${birthdayWish}\n\n` +
                        `— With love from the Shadastria Adepti fam 💙`;
        ...
        if (WELCOME_CHANNEL_ID) {
            ...
            if (welcomeChannel) {
                const welcomeMessage = 
                    `🎉 **Welcome to the Axtrix Community!** 🎉\n\n` +
                    `Hey ${member}! 👋\n\n` +
...
client.on(Events.MessageCreate, async (message) => {
        ...
        try {
            ...
            if (!state && !['!help', '!status', '!pause', '!resume', '!change time', '!change message', '!stop', '!reminder', '!clear'].includes(content)) {
                try {
                    ...
                    const systemPrompt = isFirstChat 
                        ? `You are Axtrix, a friendly Discord productivity bot. This is the user's FIRST chat with you.\n\n` +
                        ...
                        : `You are Axtrix, a friendly Discord productivity bot assistant. You remember our conversation. Respond naturally and helpfully. Be conversational and concise (under 80 words).`;
            ...
            try {
                ...
                const aiResponse = await aiService.askQuestion(
                    `You are Axtrix, a helpful productivity bot assistant. A user sent you: "${message.content}". ` +
    ...
    if (message.channel.id === GEMINI_CHANNEL_ID && !message.content.startsWith('!')) {
        try {
            ...
            const systemPrompt = `You are Axtrix, a helpful AI assistant in a Discord server. You have conversation memory and can reference previous messages. Answer questions naturally and helpfully. Keep responses concise (under 150 words).`;
    ...
    if (message.mentions.has(client.user) && !message.author.bot) {
        try {
            ...
            const systemPrompt = `You are Axtrix, a helpful AI assistant in a Discord server. A user mentioned you with a question. Answer helpfully and concisely (under 150 words). Be friendly and conversational.`;
    ...
    if (message.content === '!help') {
        const helpMessage = `🤖 **Axtrix Bot Commands**\n\n` +
            `📊 **General:**\n` +
